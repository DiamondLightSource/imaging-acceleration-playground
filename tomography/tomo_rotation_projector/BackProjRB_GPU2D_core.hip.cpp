...
__global__ void rotateIm_kernel(float *img_out, hipTextureObject_t texObj, float theta, int dimDetect)
{ 
...
extern "C" void BackProjGPU(float *sino_in, float *image_out, float *proj_angles, int dimDetect, int dimAngles)
{
    ...
    if (deviceCount == 0) {
		...
		hipArray* cuArray;
		hipMallocArray(&cuArray, &channelDesc, dimDetect, dimDetect);
		// Copy to device memory some data located at address h_data
		// in host memory
		//hipMemcpyToArray(cuArray, 0, 0, image_out, dimDetect*dimDetect*sizeof(float), hipMemcpyHostToDevice);
		
		hipMemcpy2DToArray(cuArray, 0, 0,  image_out, sizeof(image_out) * dimDetect*dimDetect, 1, 1, hipMemcpyHostToDevice);
		...
		checkCudaErrors( hipMalloc((void**)&sino_in_d,SinoSize*sizeof(float)) );
		checkCudaErrors( hipMalloc((void**)&image_out_d,ImSize*sizeof(float)) );
		checkCudaErrors( hipMalloc((void**)&image_rot_d,ImSize*sizeof(float)) );		
    
        checkCudaErrors( hipMemcpy(sino_in_d,sino_in,SinoSize*sizeof(float),hipMemcpyHostToDevice));
        hipMemset(image_out_d, 0, ImSize*sizeof(float));
        hipMemset(image_rot_d, 0, ImSize*sizeof(float));        
        ...
        hipMemcpyToArray(cuArray, 0, 0, image_rot_d, dimDetect*dimDetect*sizeof(float),hipMemcpyDeviceToDevice);        
        //hipMemcpy2DToArray(cuArray, 0, 0,  image_rot_d, sizeof(image_rot_d) * dimDetect*dimDetect, 1 , 1, hipMemcpyDeviceToDevice);
	...
	hipFreeArray(cuArray);    
        ...
        hipMemcpy(image_out,image_out_d,ImSize*sizeof(float),hipMemcpyDeviceToHost);
